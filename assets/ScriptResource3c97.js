...
else if (navigator.userAgent.indexOf('AppleWebKit/index.html') > -1) {
...
Sys.Net.WebRequest._resolveUrl = function Sys$Net$WebRequest$_resolveUrl(url, baseUrl) {
    ...
    baseUrl = baseUrl.substr(0, baseUrl.lastIndexOf('index.html') + 1);
    if (!url || url.length === 0) {
        return baseUrl;
    }
    if (url.charAt(0) === 'index.html') {
        ...
        var nextSlash = baseUrl.indexOf('index.html', slashslash + 3);
        if (nextSlash === -1) {
        ...
        var lastSlash = baseUrl.lastIndexOf('index.html');
        if (lastSlash === -1) {
